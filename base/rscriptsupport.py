...
class RScripInterface(object):
    """ Interface class for R: call Rscript, read and write input/output
    data."""
    ...
    def curve_files_uptodate(self, prefix='all'):
        ...
        if os.path.isfile(os.path.join(self.output, prefix + '_group.csv')) or\
           os.path.isfile(os.path.join(self.output, prefix + '.csv')):
            return False
        return True

    def store_for_r(self, curves, prefix='all'):
        """ Store dictionary of curve lists: curve per file and all curves to
        a single file with prefix + prefix_group.csv (for preserving
        groups)."""
        ...
        with open(all_group_filename, 'w') as all_group_csvfile,\
                open(all_filename, 'w') as all_csvfile:
            all_groupwriter = csv.writer(all_group_csvfile, delimiter=';',
                                         quotechar='|',
                                         quoting=csv.QUOTE_MINIMAL)
            allwriter = csv.writer(all_csvfile, delimiter=';', quotechar='|',
                                   quoting=csv.QUOTE_MINIMAL)
            for category, curve_list in curves.items():
                for curve in curve_list:
                    curve_line = list(itertools.chain.from_iterable(curve))
    ...
    def write_csv(self, filename, grouped_data):
        ...
        for group_name, group in grouped_data.items():
            for row in group:
                writer.writerow([order, group_name] +
